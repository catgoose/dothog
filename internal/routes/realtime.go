...
func (ar *AppRoutes) initRealtimeRoutes(broker *tavern.SSEBroker) {
	...
	numBroker = broker
	initRTIntervals()
	dash := ar.e.Group("/realtime/dashboard")
	dash.GET("", ar.handleRealtimePage())
	dash.POST("/interval", handleRTInterval)
	dash.POST("/interval-all", handleRTIntervalAll)
	dash.POST("/interval-restore", handleRTIntervalRestore)
	dash.POST("/pin", handleRTPin)
	sse := ar.e.Group("/sse")
	sse.GET("/system", echo.WrapHandler(broker.SSEHandler(TopicSystemStats)))
	sse.GET("/dashboard", echo.WrapHandler(broker.SSEHandler(TopicDashMetrics)))

	// Numerical tile publisher (shares the page, separate SSE stream)
	initTileIntervals()
	dash.POST("/tile-interval", handleNumericalInterval)
	sse.GET("/numerical", echo.WrapHandler(broker.SSEHandler(TopicNumericalDash)))
